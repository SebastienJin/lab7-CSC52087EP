...
class DDPMSampler(nn.Module):
    ...
    def sample(
        ...
        for step, (g_cur, g_next) in enumerate(zip(gammas[:-1], gammas[1:])):
            ...
            var_t = (1 - g_next) * (1 - alpha_cur) / (1 - g_cur)
            x_mean = torch.sqrt(g_next) * x0 + torch.sqrt(1 - g_next - var_t) * (x_cur - torch.sqrt(g_cur) * x0) / torch.sqrt(1 - g_cur)
            x_next = x_mean + randn_like(x_cur) * torch.sqrt(var_t)
            # ------------------------------------------------------------------------- #
